...
FUNCTION /DMO/FLIGHT_TRAVEL_CALC_PRI_03.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
*"     REFERENCE(IT_FLIGHT) TYPE  /DMO/IF_FLIGHT_LEGACY03=>TT_FLIGHT
*"       OPTIONAL
*"----------------------------------------------------------------------
  TYPES BEGIN OF ty_seats_occupied.
  INCLUDE TYPE /dmo/flight03.
  TYPES   distance       TYPE /dmo/connecti_03-distance.
  TYPES   distance_unit  TYPE /dmo/connecti_03-distance_unit.
  ...
  IF it_flight IS SUPPLIED.
    SELECT FROM /dmo/booking03 AS booking
      JOIN     /dmo/connecti_03 AS connection
                 ON  booking~carrier_id    = connection~carrier_id
                 AND booking~connection_id = connection~connection_id
      JOIN     /dmo/flight03 AS flight
